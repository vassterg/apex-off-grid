import { finishLongops, registerLongops } from "../longopsPopup";
...
export async function cachAppPages(pageNames: string[]) {
	...
	if (split[3] !== "ords") {
		window.apex.debug.warn(
			"Did not find /ords/ in url, skipping cache page",
			split,
		);
		return;
	} else if (split.length !== 8) {
		window.apex.debug.warn(
			"Url does not have 8 parts, skipping cache page",
			split,
		);
		return;
	}
	...
	const longopsKey = `Cache ${pageNames.length} pages`;
