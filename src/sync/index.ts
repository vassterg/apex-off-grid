...
import { finishLongops, registerLongops, updateLongops } from "../longopsPopup";
...
export async function syncRows({
	...
	const longopsKey = `Sync ${storageId} v${storageVersion}`;
	try {
		registerLongops(longopsKey);
		updateLongops(longopsKey, "Send device changes to server");
		...
		while (hasMoreRows) {
			iter += 1;
			updateLongops(longopsKey, `Fetch DB rows (${iter})`);
		...
		apex.debug.error(`Error syncing rows: ${e.message}`);
	} finally {
		finishLongops(longopsKey);
	}
}
